function [ImportedData, ChannelMat, nChannels, nTime, ImportOptions, DateOfStudy] = in_data_MOTOR_LAB( DataFile, ChannelMat, FileFormat, ImportOptions, nbCall)
...
if isRaw
    ...
    nTime     = zeros(1,length(BlocksToRead));
    nChannels = zeros(1,length(BlocksToRead));
    parfor iFile = 1:length(BlocksToRead)
        [ImportedData_single, nTime_single, nChannels_single] = import_in_parallel(iFile, DataFile, BlocksToRead, ImportOptions, initBaselineRange, sFile, ChannelMat, badSeg, tmpDir);
        nTime(iFile) = nTime_single;
        nChannels(iFile) = nChannels_single;
        ImportedData(iFile) = ImportedData_single;
    end
...
function [ImportedData_single, nTime_single, nChannels_single] = import_in_parallel(iFile, DataFile, BlocksToRead, ImportOptions, initBaselineRange, sFile, ChannelMat, badSeg, tmpDir)

    % Initialize returned variables
    ImportedData_single = repmat(db_template('Data'), 0);
    ...
    ImportedData_single(end+1).FileName = newFileName;
    ImportedData_single(end).Comment    = DataMat.Comment;
    ImportedData_single(end).DataType   = DataMat.DataType;
    ImportedData_single(end).BadTrial   = isBad;
    % Count number of time points
    nTime_single = length(TimeVector);
    nChannels_single = size(DataMat.F,1);
