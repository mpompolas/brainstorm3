function varargout = process_ssp2_MOTOR_LAB( varargin )
...
function sProcess = GetDescription()
    ...
    sProcess.Comment     = 'SSP: Generic - PARRALELIZED';
...
function OutputFiles = Run(sProcess, sInputsA, sInputsB)
    ...
    if isfield(sProcess.options, 'resample_factor') && isfield(sProcess.options.resample_factor, 'Value') && ~isempty(sProcess.options.resample_factor.Value)
        resample_factor = sProcess.options.resample_factor.Value{1};
        strOptions = [strOptions, 'resample_factor=' num2str(resample_factor) ', '];
    else
        resample_factor = 1;
    end
    
    % Check if it should run in parallel
    run_parallel = sProcess.options.paral.Value;
...
%             end
    ...
    if (resample_factor > 1)
        ...
        % PARALLELIZE HERE
        if run_parallel
            disp('Downsampling in parallel')
            tic
            parfor iFile = 1:length(F)
                % This just downsamples by a factor. Should be faster that the
                % normal process_resample function that is normally used
                F{iFile} = downsample(F{iFile}',resample_factor)';
            end
            disp(['Parallel downsampling took: ' num2str(toc) ' seconds'])
        else
            disp('Downsampling without parallel support')
            tic
            for iFile = 1:length(F)
                % This just downsamples by a factor. Should be faster that the
                % normal process_resample function that is normally used
                F{iFile} = downsample(F{iFile}',resample_factor)'; % The inputs to downsample have to be inverted and the output inverted again to make sense
            end
            disp(['Non - Parallel downsampling took: ' num2str(toc) ' seconds'])
        end
            
        TimeResample = downsample(TimeVector,resample_factor);
        sfreq = DataMat.F.prop.sfreq/resample_factor;
